...
from models.faenet.model import FAENet
...
def train():
    ...
    for target_index, target_name in zip(target_indices, target_names):
        ...
        # Example: Approximate atomic volumes in Ångströms³ (empirical or van der Waals)
        atomic_volumes = {
        1: 5.24,   # Hydrogen
        6: 20.58,  # Carbon
        7: 17.30,  # Nitrogen
        ...
        # Attach atomic volume to each molecule
        for data in dataset_list:
            if hasattr(data, 'z'):  # Check if atomic numbers exist
                try:
                    volumes = torch.tensor(
                        [atomic_volumes[int(zi)] for zi in data.z],
                        dtype=torch.float
                    )
                    data.atomic_volume = volumes  # Attach to the Data object
                except KeyError as e:
                    print(f"Warning: atomic number {e} not in volume table for molecule {data.name}")
                    data.atomic_volume = torch.zeros_like(data.z, dtype=torch.float)  # fallback
        ...
        for fold, (train_idx, test_idx) in enumerate(kf.split(dataset)):
            ...
            for epoch in range(1, epochs + 1):
                ...
                for data in train_loader:
                    ...
                    loss = criterion(target_pred['energy'].squeeze(), data.y[:, target_index])
                ...
                with torch.no_grad():
                    for data in val_loader:
                        ...
                        val_loss = criterion(target_pred['energy'].squeeze(), data.y[:, target_index])
