...
from models.dimenet import DimeNetMulti
...
def train():
    ...
    model_dir = 'dimenet_pp_multimodal'
    ...
    for target_index, target_name in zip(target_indices, target_names):
        ...
        for fold, (train_idx, test_idx) in enumerate(kf.split(dataset)):
            ...
            # Initialize the DimeNetMulti model with appropriate parameters.
            model = DimeNetMulti(
                hidden_channels=32,
                num_blocks=2,
                int_emb_size=32,
            ...
            for epoch in range(1, epochs + 1):
                ...
                for data in train_loader:
                    ...
                    optimizer.zero_grad()
                    # Forward pass:
                    # Here we pass CLIP embeddings if available (adjust the attribute name if needed).
                    target_pred = model(data.z, data.pos, data.batch, data.clip_embedding)

                    # Calculate prediction loss (make sure data.y is shaped correctly).
